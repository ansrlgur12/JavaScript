...
// slice() : 시작인덱스부터 종료인덱스 미만까지의 배열 요소를 잘라냄
let fruits10 = ["Banana", "Orange", "Apple", "Mango", "Lemon"];
let slice1 = fruits10.slice(3); // 3부터 끝까지
let slice2 = fruits10.slice(1, 3); // 1번부터 3번 미만까지
console.log(slice1);
console.log(slice2);
...
// filter() : 배열에서 특정 조건을 만족하는 배열의 요소만을 찾아서 
// 새로운 배열 생성
let persons = [
    {
        name : "유재석",
...
// 인스턴스 만들기
const date1 = new Date("2022-05-05/05:30:50");
console.log(date1);
...
console.log(myDate.getTime()); // 1970년 1월1일 0시0분부터 지금까지의 시간경과

// Map 객체 : 자바스크립트에서 제공하는 데이터 구조 중 하나로 key - value 쌍으로 데이터를 저장 관리 합니다.
// Map은 객체와 유사하지만 객체와는 다르게 키에 다양한 자료형을 사용 할 수 있다.
let map = new Map();
map.set("name", "유나");
...
// Math.random() : 0보다 크고 1보다 작은 임의의 숫자형 값을 반환
...
// 응모자 수 : 30명
// 당첨자 발표 : 응모자 수 사이에서 당첨자를 선택(랜덤함수이용)해 발표